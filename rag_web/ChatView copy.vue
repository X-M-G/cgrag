...
const isStreaming = ref(false) // 新增：全局是否在流式输出中
...
const sendMessage1 = async () => {
  ...
  if (activeChatIndex.value === -1) {
    chatHistory.value.unshift({ title: '新对话', id: Date.now(), messages: [] })
    activeChatIndex.value = 0
  }
  let currentChat = chatHistory.value[activeChatIndex.value]
  if (!currentChat) {
    // 兜底
    chatHistory.value.unshift({ title: '新对话', id: Date.now(), messages: [] })
    activeChatIndex.value = 0
    currentChat = chatHistory.value[0]
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      // 创建助手消息：初始内容为空，标记为 streaming
      const assistantMsg: ChatMessage = {
        role: 'assistant',
        content: '',
        timestamp: new Date().toLocaleTimeString(),
        images: data.images, // 暂存，v-if会隐藏它们
      ...
      // 3. 执行打字机动画
      await simulateTypewriter(assistantMsg, data.answer)
...
// ----------------------------------------------------------------
// 1. 模拟打字机 (使用 requestAnimationFrame 确保不卡顿)
// ----------------------------------------------------------------
  ...
  return new Promise<void>((resolve) => {
    ...
    const typeNext = () => {
      ...
      if (currentIndex >= totalLength) {
        msg.isStreaming = false // 关闭流式（光标消失，资料显示）
        nextTick(() => {
          scrollToBottom()
          autoResize()
        })
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      // 3. 🔥🔥🔥 关键：从数组中取出 Vue 生成的响应式代理对象 (Proxy) 🔥🔥🔥
      // 如果你直接修改 rawAssistantMsg，Vue 是监听不到的！必须修改数组里的这个！
